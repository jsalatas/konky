...
-- 	end
    local current_max = 1
     for i in pairs(graph_settings) do
         if graph_settings[i].automax ~= nil then 
            current_max = math.max(graph_settings[i].automax, current_max)
         end 
     end
    t.max = current_max * 1.1
...
function conky_kib_to_bit(value)
    local out = conky_parse( value) * 8 * 1024
    ...
    if out > 1000 then
     ...
     return string.format("%.1f", out ).." "..unit..'bps'
end
